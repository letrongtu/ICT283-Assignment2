...
void thirdCaseHandler(const WeatherLog& weather_data) {
    ...
    cout << "-------------------------------------------------------------------------------------------------------------------" << endl;
    Vector<double> solarRadiations;
    for (int month = 1; month <= MonthCounts; month++) {

        for (int i = 0; i < weather_data.Size(); i++) {
            if (weather_data[i].date.GetMonth() == month && weather_data[i].date.GetYear() == year && weather_data[i].solarRad > 100) {
                solarRadiations.add(weather_data[i].solarRad * RadiationExchangeRate);
            }
        }

        thirdCaseOutput(solarRadiations, month, year);
...
void thirdCaseOutput(const Vector<double>& solarRadiations, int month, int year){
    if (solarRadiations.Size() == 0) {
            outputSolarRad(0, month);
        } else {
            double totalRadiation = calculateTotal(solarRadiations);
            outputSolarRad(totalRadiation, month);
        }
}
